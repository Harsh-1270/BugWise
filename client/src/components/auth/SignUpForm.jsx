...
export default function SignUpForm({ onSignUpSuccess, onSwitchToSignIn, onClose }) {
  ...
  try {
    ...
    if (!window.google) {
      toast.dismiss(loadingToast)
      showThemedToast.error('Google Sign-In library not loaded. Please refresh and try again.')
      return
    }

    // Initialize Google OAuth with proper configuration
    window.google.accounts.id.initialize({
      client_id: import.meta.env.VITE_GOOGLE_CLIENT_ID || '457943380295-0tdjksgk2nmoiks6naouim9scd1868rd.apps.googleusercontent.com',
      callback: async (response) => {
        try {
          // Send the credential to your backend for verification
          const {data: responseData} = await axios.post('/google', {
          ...
          if (responseData.error) {
            ...
            const userData = {
              ...
              id: responseData.id || responseData.user?.id,
              picture: responseData.picture || userInfo.picture
            }
    ...
    // Use popup mode instead of One Tap
    window.google.accounts.id.prompt((notification) => {
      if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
        toast.dismiss(loadingToast)
        // Fallback: Show a message to the user
        showThemedToast.error('Google Sign-In popup was blocked. Please enable popups and try again.')
      } else {
        toast.dismiss(loadingToast)
      }
    })
    ...
    showThemedToast.error('Google signup failed. Please try again.')
  }
}
